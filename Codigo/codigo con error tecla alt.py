        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    # Agregar la posición del índice al historial
                    historial_posiciones.append((index_x, index_y))
                    if len(historial_posiciones) > 10:
                        historial_posiciones.pop(0)

                    # Detectar si se ha realizado el gesto de pulsación
                    if detectar_gesto(historial_posiciones):
                        tecla = detectar_tecla(index_x, index_y)
                        if tecla:
                            manejar_tecla(tecla)
